...
create table tt (N INT, id_TT INT, name_TT varchar(40), Ñòàòóñ varchar(10),tt_format INT, region_tt varchar(20), city_tt varchar(20));

insert into tt values 
(24574, 14858, '24574ÌÒ_Ëåíèíãðàäñêîå', 'Îòêðûò', 2, 'Ìîñêâà', 'Ìîñêâà'),
(24579, 14863, '24579ÄÑ_Êàâêàçñêèé', 'Îòêðûò', 1, 'Ìîñêâà', 'Ìîñêâà'),
(24580,	14864,	'24580ÄÑ_Âøê_ÒÏÇÀëòóôüåâî',	'Çàêðûò',	2,	'ÌÎ',	'ÌÎ'),
(22566,	12872,	'22566Ì_Ëåíèíãðàäñêèé',	'Çàêðûò',	2,	'Ðåãèîíû',	'Òâåðü'),
(24805,	15094,	'24805ÄÑ_Ëîáà÷åâñêîãî',	'Îòêðûò',	3,	'Ðåãèîíû',	'Òàìáîâ');
...
INNER JOIN dt 
        on tt.id_tt = dt.id_tt
where name_TT LIKE '%ÄÑ_%' and date_tt < '2021-07-01' and date_tt >= '2021-06-23' and region_tt = 'Ìîñêâà'
...
region_tt,
CASE 
        WHEN region_tt = 'Ìîñêâà' THEN 'Ìîñêâà'
        WHEN region_tt = 'ÌÎ' THEN 'ÌÎ'
        WHEN region_tt = 'Ñàíêò-Ïåòåðáóðã' THEN 'Ñàíêò-Ïåòåðáóðã'
        WHEN region_tt = 'Ëåíèíãðàäñêàÿ îáëàñòü' THEN 'Ëåíèíãðàäñêàÿ îáëàñòü'
...
AND descr_format IN ('Àéñ', 'Âêóñ âèëë GO', 'ÂêóñÂèëë', 'Ãëîáî', 'Æóê', 'Øìåëü')
...
SELECT TOP 1 (SUM(CASE descr_format WHEN 'ÂêóñÂèëë' then summa else 0 end) OVER(partition BY descr_format))/(SUM(summa) over())*100
FROM dt
